...
#define APPLICATION_NAME "Rufus macOS"

static macos_rufus_drive drives[MAX_DRIVES];
...
macos_rufus_drive *find_device_by_name(const char *device_name) {
    // Refresh device list
    if (!macos_get_usb_devices(drives, &num_drives)) {
        return NULL;
    }
...
bool format_device(const char *device_name, const char *fs_type, const char *label) {
    macos_rufus_drive *drive = find_device_by_name(device_name);
